...
shop1_search_loop:
    ...
    mov cx,100
    lea si,offset nullcmp
    mov di,bx
    repne cmpsb
    mov ax,99
    sub ax,cx
    mov cx,ax
    lea si,offset in_good
    mov di,bx
    repe cmpsb
    cmp cx,0
    jz shop1_prefound
...
shop2_search_loop:
    ...
    lea si, offset in_good
    mov di, bx
    repe cmps byte ptr[in_good], byte ptr[bx]
    cmp cx,0
    jz shop2_prefound
    add bx,ax
    add bx,11
...
shop2_prefound:
    ...
    lea si, offset nullcmp
    lea di, offset in_good
    repne cmps byte ptr[nullcmp], byte ptr[in_good]
    mov ax,99
    sub ax,cx
    cmp ax,dx
    jz shop2_found
...
shop2_found:
    ...
    cmp al,0
    jnz admin_mode_cont
    cmp word ptr[buycnt],buymultiplier
    jz final_bought
    inc word ptr[buycnt]
    jmp retry_purch
final_bought:
